"""
avatarpipeline.engines.tts.mlx — Local MLX voice cloning and voice conversion.
...
from avatarpipeline.core.media import audio_duration, resample_audio
...
class MlxVoiceStudio:
    ...
    def list_voices(self) -> list[str]:
        """Return saved MLX voice choices for the shared TTS protocol."""
        return self.list_voice_choices()
    ...
    def generate(
        ...
        """Generate speech with a saved voice for the shared TTS protocol."""
        if not voice:
            choices = self.list_voice_choices()
            if not choices:
                raise ValueError("No saved MLX voices are available.")
            voice = choices[0]
        return self.synthesize_with_voice(text=text, voice_choice=voice, output_path=out_path)

    def convert_to_16k(self, wav_path: str, out_path: str) -> str:
        """Convert audio to 16 kHz mono PCM WAV for the shared TTS protocol."""
        resample_audio(wav_path, out_path, sample_rate=16_000, channels=1)
    ...
    def _convert_audio(self, src: Path, dest: Path, sample_rate: int) -> None:
        resample_audio(src, dest, sample_rate=sample_rate, channels=1)
