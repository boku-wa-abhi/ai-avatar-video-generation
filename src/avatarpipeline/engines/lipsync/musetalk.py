"""
avatarpipeline.engines.lipsync.musetalk — MuseTalk 1.5 lip-sync inference wrapper.
...
class MuseTalkInference:
    ...
    def __init__(self) -> None:
        ...
        self.musetalk_dir = Path(os.path.expanduser(str(self.config.musetalk_dir)))
        self.venv_python = self.musetalk_dir / "musetalk-env" / "bin" / "python"
        self.default_fps = self.config.musetalk_fps
        self.default_batch_size = self.config.musetalk_default_batch_size
        self.default_bbox_shift = self.config.musetalk_default_bbox_shift
    ...
    def run(
        ...
        try:
            cmd = [
                ...
                "--batch_size", str(batch_size if batch_size is not None else self.default_batch_size),
                "--bbox_shift", str(bbox_shift if bbox_shift is not None else self.default_bbox_shift),
