"""
avatarpipeline.narration.presenter — Persistent slide narrator with presenter lip-sync.
...
from avatarpipeline.narration.slide_renderer import render_slides
...
def _video_info(path: str | Path) -> dict:
    cmd = [
        "ffprobe", "-v", "error",
        "-select_streams", "v:0",
    ...
    try:
        ...
        return {"width": 1920, "height": 1080, "duration": 0.0, "fps": 25.0}


def _gen_silence(duration: float, output_path: str | Path) -> None:
...
def _concat_audio(paths: list[str | Path], output_path: str | Path) -> None:
    inputs: list[str] = []
    for p in paths:
        inputs += ["-i", str(p)]
    cmd = [
